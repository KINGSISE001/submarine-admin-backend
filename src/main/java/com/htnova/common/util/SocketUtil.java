...
public class SocketUtil {
     ...
     */
    public static String getHttpSessionId(String cookieStr) {
    ...
    public static String getHttpSessionId2(String cookieStr) {
        final String SESSION_NAME = "SESSION";
        StringBuilder sb = new StringBuilder();
        sb.append(SESSION_NAME).append("=");
        sb.append(cookieStr);
        if (StringUtils.isBlank(cookieStr)) return null;
        String sessionId = new String(Base64.getDecoder().decode(sb.substring(SESSION_NAME.length() + 1)));
        if (StringUtils.isBlank(sessionId)) {
            return null;
       }
                return sessionId;
